...
el("playBtn").onclick = async () => {
  ...
  const aceChoice = el("aceChoice");
  const aceValue = aceChoice.classList.contains("hidden") ? undefined : Number(aceChoice.dataset.value || 1);
...
function renderHand(){
  const title = el("mobileActionTitle");
  if(title){
    const isMyTurnTitle = state?.currentPlayerId === me?.id && !state?.pendingDistribution && !(state?.death?.active && !state?.death?.finished);
    title.textContent = isMyTurnTitle ? "À toi de jouer" : `Tour de ${state?.currentPlayerName || "..."}`;
  }
  ...
  me.hand.forEach(card=>{
    ...
    div.onclick = () => {
      ...
      const aceChoice = el("aceChoice");
      aceChoice.classList.toggle("hidden", card.rank !== "A");
      if(card.rank === "A"){
        aceChoice.dataset.value = "1";
        el("aceOneBtn").classList.add("active");
        el("aceElevenBtn").classList.remove("active");
      }
      renderHand();
    };
...
function renderDeath(){
  ...
  el("newRoundBtn").classList.toggle("hidden", !state.death.finished);
  ...
  const fullGrid = state.death.finished && state.death.grid ? state.death.grid : null;
  rows.forEach((size,r)=>{
    if(size===1){
      const wrap = document.createElement("div");
      wrap.className = "death-row-single";
      wrap.appendChild(makeDeathCell(r,0,isMine,revealedMap,fullGrid));
      grid.appendChild(wrap);
    } else {
      for(let c=0;c<4;c++) grid.appendChild(makeDeathCell(r,c,isMine,revealedMap,fullGrid));
    }
  });
}
function makeDeathCell(r,c,isMine,revealedMap,fullGrid=null){
  const cell = document.createElement("div");
  const card = revealedMap.get(`${r}-${c}`) || (fullGrid ? fullGrid[r]?.[c] : null);
  const isRevealedByEnd = fullGrid && card && !revealedMap.has(`${r}-${c}`);
  const isHead = card && HEADS.includes(card.rank);
  cell.className = `death-cell ${card ? "revealed" : ""} ${isHead ? "head" : card ? "safe" : ""} ${isRevealedByEnd ? "avoided" : ""} ${isMine ? "" : "disabled"}`;
...
// V6 : choix de l'As avec deux gros boutons
function setAceChoice(value){
  const box = document.getElementById("aceChoice");
  if(!box) return;
  box.dataset.value = String(value);
  document.getElementById("aceOneBtn")?.classList.toggle("active", value === 1);
  document.getElementById("aceElevenBtn")?.classList.toggle("active", value === 11);
}
document.getElementById("aceOneBtn")?.addEventListener("click", () => setAceChoice(1));
document.getElementById("aceElevenBtn")?.addEventListener("click", () => setAceChoice(11));
